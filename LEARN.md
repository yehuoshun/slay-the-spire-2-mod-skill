# 学习流程

当老板说"学习"或发来教程链接时，按以下流程执行：

---

## 学习步骤

1. **读内容** — 读老板发的链接/文档/代码
2. **提炼关键知识** — 提取可复用的代码模板、API 用法、常见坑
3. **写 references** — 创建 `references/<模块名>.md`，结构：
   - 概述
   - 代码模板（标注引用来源）
   - API 速查
   - 常见问题
4. **标注演进路线** — 当前方案如果还有更优方案（如硬注册 → 一键注册），在末尾用 ## 演进路线 注明，方便后期替换
5. **更新 SKILL.md** — 参考资料表加新行
6. **更新 README.md** — 状态列表勾选 + 鸣谢追加
7. **版本管理** — 新学内容写 `references/<模块>/<模块>-v2.md`（原版不动），审查通过后：原版移至 [slay-the-spire-2-mod-skill-archive](https://github.com/yehuoshun/slay-the-spire-2-mod-skill-archive) 存档，更新 `xx.md` 导航入口指向最新版
8. **commit + push**

## 重要

- 当前学的方案可能不是最优解，后面学到更好的要在 reference 末尾加「演进路线」说明
- 不要为了追求完美跳过当前学习，先学再优化

---